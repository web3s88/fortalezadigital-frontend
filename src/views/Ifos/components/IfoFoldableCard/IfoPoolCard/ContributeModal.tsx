...
const ContributeModal: React.FC<Props> = ({
  ...
  const label = currency === tokens.cake ? t('Max. Duro entry') : t('Max. token entry')
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    poolId === PoolIds.poolBasic
      ? t(
          'For the basic sale, Max Duro entry is capped by minimum between your average Duro balance in the IFO Duro pool, or the pool’s hard cap. To increase the max entry, Stake more Duro into the IFO Duro pool',
        )
      : t(
          'For the unlimited sale, Max Duro entry is capped by your average Duro balance in the IFO Duro pool. To increase the max entry, Stake more Duro into the IFO Duro pool',
        ),
    {},
  )
      ...
      <ModalBody maxWidth="360px">
        <Box p="2px">
          ...
          <Text color="textSubtle" fontSize="12px" mb="24px">
            {t(
              'If you don’t commit enough Duro, you may not receive any IFO tokens at all and will only receive a full refund of your Duro.',
            )}
            <Link
              fontSize="12px"
              display="inline"
