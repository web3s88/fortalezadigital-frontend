...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const tooltipContent =
    poolId === PoolIds.poolBasic
      ...
      : t(
          'For the unlimited sale, Max Duro entry is capped by your average Duro balance in the IFO Duro pool. To increase the max entry, Stake more Duro into the IFO CAEK pool',
        )

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. Duro entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('Duro to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
